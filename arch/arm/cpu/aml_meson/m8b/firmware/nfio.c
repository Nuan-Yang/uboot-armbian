...
STATIC_PREFIX unsigned page_convert_slc(unsigned page)
{
	...
	if (page0_info->new_nand_type) {		/* for new nand */
		if (page0_info->new_nand_type < 10) {   /* for hynix slc mode */
			if (page0_info->new_nand_type == 6)
				read_page = pagelist_1ynm_hynix256[page%page0_info->pages_in_block] \
					    + (page/page0_info->pages_in_block)*page0_info->pages_in_block;
			else
				read_page = pagelist_hynix256[page%page0_info->pages_in_block] \
					    + (page/page0_info->pages_in_block)*page0_info->pages_in_block;
		}
		else if (page0_info->new_nand_type == 40) {  /* for sandisk slc mode */
			read_page = (page%page0_info->pages_in_block)*2 + (page/page0_info->pages_in_block)*page0_info->pages_in_block;
		}
	}
	return read_page;
}
...
STATIC_PREFIX short nfio_page_read(unsigned src, unsigned mem, unsigned char *oob_buf, unsigned ext)
{
	...
	read_page = page_convert_slc(src);
...
STATIC_PREFIX int nf_init(unsigned ext, unsigned *data_size)
{
	...
	pages = cfg->ext&0x3f;
	page_size = short_mode ? short_size : (ecc_mode < 2 ? 64 : 128);
